...
realvars <- c("Kicks", "Handballs", "Disp_eff", "Frees_Agst", "Goals", "Behinds")
catvars <- c("Club")
...
server <- function(input, output) {
  ...
  output$mds <- renderPlotly({
   ...
   #  players_mds_df18$Player <- players$Player[players$Year == "2018"]
     ...
     # Make the plot
     mds <- ggplot(players_mds_df, aes(x=V1, y=V2, label=Club)) + facet_wrap(~Year, scales="free")
