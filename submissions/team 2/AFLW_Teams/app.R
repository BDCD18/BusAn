...
realvars <- c("Kicks", "Handballs", "Frees_For", "Frees_Agst", "Goals", "Behinds", "Disp_eff")
catvars <- c("Club")
teams$Year <- as.factor(teams$Year)
clubs <- unique(teams$Club)

# Define UI for application
ui <- fluidPage(theme = shinytheme("flatly"),
  titlePanel("Women's AFL Statistics Exploration - Team Analysis"),
    tabsetPanel(
      ...
      tabPanel("MDS",
               ...
               sidebarLayout(
                 ...
                 mainPanel(
                   plotlyOutput("mds_plot", height="400px")
                 )
               )
      ),
      tabPanel("Basic Plots",
               ...
               sidebarLayout(
                 sidebarPanel(
                   #selectInput('x', "X", realvars, realvars[1]),
                   selectInput('y_2', "Y", realvars, realvars[2])
                   #selectInput('label', "Label", catvars),
                   #checkboxGroupInput('clr1', "Colour by club:", clubs, NA)
                 ),
...
server <- function(input, output) {
  ...
  output$mds_plot <- renderPlotly({
    ...
    if (length(input$clr1) > 0) {
      players_club <- teams %>% filter(Club %in% input$clr1)
  ...
  output$scatterplot <- renderPlotly({
     ...
     # facetted by year
     p <- ggplot(teams,
            aes_string(x = 'Club', y = input$y_2,
                       fill='Year')) +
            geom_col(position="dodge") + scale_fill_discrete(name="Year")
       #facet_wrap(~Year, ncol=2)
