...
players <- read_csv("data/teams.csv")
realvars <- colnames(players)[-c(1,2)]
...
server <- function(input, output) {
  ...
  output$scatterplot <- renderPlotly({
    ...
    if (length(input$clr1) > 0) {
      ...
      p <- p + geom_boxplot(alpha = 0.1, size=1) +
        geom_boxplot(data=players_club, mapping=aes(colour=Club),
                     alpha=1, size=2) +
        scale_colour_brewer(palette="Dark2") +
        theme(legend.position = "bottom")
    }
    else # If no clubs chosen make all points black
      p <- p + geom_boxplot(alpha = 0.4)
  ...
  output$mds <- renderPlotly({
    ...
    # Make the plot
    mds <- ggplot(players_mds_df, aes(x=V1, y=V2, label=Club)) +
